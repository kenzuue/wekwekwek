...
<body>
    ...
    if(isset($_SESSION["user"])){
        if(($_SESSION["user"])=="" or $_SESSION['usertype']!='b'){
    ...
    $userrow = $database->query("select * from barber where baremail='$useremail'");
    $userfetch=$userrow->fetch_assoc();
    $userid= $userfetch["barid"];
    $username=$userfetch["barname"];
 //echo $userid;
 ?>
 <div class="container">
     <div class="menu">
     <table class="menu-container" border="0">
             ...
             <tr class="menu-row" >
                 <td class="menu-btn menu-icon-patient">
                     <a href="customer.php" class="non-style-link-menu"><div><p class="menu-text">My Customers</p></a></div>
                 </td>
             </tr>
             <tr class="menu-row" >
            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <tr>
                    <td colspan="4" style="padding-top:0px;width: 100%;" >
                        ...
                        <td width="30%">
                        <select name="barid" id="" class="box filter-container-items" style="width:90% ;height: 37px;margin: 0;" >
                            <option value="" disabled selected hidden>Choose Barber Name from the list</option><br/>
                                
                            <?php 
                            
                                $list11 = $database->query("select  * from  barber order by barname asc;");
                ...
                <?php
                    if($_POST){
                        ...
                        if(!empty($_POST["barid"])){
                            $barid=$_POST["barid"];
                            $sqlpt2=" barber.barid=$barid ";
                        }
                        //echo $sqlpt2;
                        //echo $sqlpt1;
                        $sqlmain= "select schedule.scheduleid,schedule.title,barber.barname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join barber on schedule.barid=barber.barid ";
                        ...
                        $sqlmain= "select schedule.scheduleid,schedule.title,Barber.barname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join barber on schedule.barid=barber.barid  order by schedule.scheduledate desc";
    ...
    if($_GET){
        ...
        if($action=='add-session'){
                    ...
                    <center>
                        ...
                        <tr>
                            ...
                            <tr>
                                ...
                                <td class="label-td" colspan="2">
                                    <label for="barid" class="form-label">Select Barber: </label>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
                                    <select name="barid" id="" class="box" >
                                    <option value="" disabled selected hidden>Choose Barber Name from the list</option><br/>';
            ...
            $sqlmain= "select schedule.scheduleid,schedule.title,barber.barname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join barber on schedule.barid=barber.barid  where  schedule.scheduleid=$id";
            $result= $database->query($sqlmain);
            $row=$result->fetch_assoc();
            $barname=$row["barname"];
            $scheduleid=$row["scheduleid"];
            ...
            $sqlmain12= "select * from appointment inner join customer on customer.cid=appointment.pid inner join schedule on schedule.scheduleid=appointment.scheduleid where schedule.scheduleid=$id;";
